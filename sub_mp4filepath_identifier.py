"""
影音檔案路徑識別模組
負責從HTML檔案中提取m3u8路徑，查找對應的影音檔案，並更新活動類型和路徑
包含完整的日誌記錄功能
"""
...
def setup_logger():
    """設置日誌記錄器"""
    logger = logging.getLogger('media_identifier')
    if not logger.handlers:
        # 創建日誌檔案名（包含時間戳）
        log_filename = f"media_identifier_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def extract_filename_from_m3u8_url(m3u8_url):
    ...
    try:
        ...
        # 方法1：使用正則表達式查找常見影音檔案格式前的檔案名
        for ext in CONFIG['VIDEO_EXTENSIONS'] + CONFIG['AUDIO_EXTENSIONS']:
            pattern = rf'/([^/]+)\{ext}/'
            match = re.search(pattern, path, re.IGNORECASE)
            if match:
                filename = match.group(1)
                logger.info(f"使用正則表達式提取檔案名: {filename} (擴展名: {ext})")
                return filename
        
        # 方法2：如果正則失效，嘗試分割路徑查找包含影音檔案擴展名的部分
        path_parts = path.split('/')
        for part in path_parts:
            for ext in CONFIG['VIDEO_EXTENSIONS'] + CONFIG['AUDIO_EXTENSIONS']:
                if ext.lower() in part.lower() and not part.endswith('.m3u8'):
                    # 移除副檔名
                    filename = part.replace(ext.lower(), '').replace(ext.upper(), '')
...
def find_media_file(base_path, filename):
    ...
    try:
        ...
        for level in CONFIG['PATH_LEVELS']:
            ...
            logger.info(f"開始在路徑中搜索影音檔案: {search_path}, 目標檔案名: {filename}")
            ...
            for root, dirs, files in os.walk(search_path):
                for file in files:
                    ...
                    if file_base.lower() == filename.lower():
                        ...
                        # 檢查是否為影片類型（不區分大小寫）
                        video_extensions_lower = [ext.lower() for ext in CONFIG['VIDEO_EXTENSIONS']]
                        if file_ext in video_extensions_lower:
                            found_files.append({
                                'type': 'video',
                                'path': full_path,
                                'level': level,
                            ...
                            logger.info(f"找到影片檔案: {full_path}")
                        
                        # 檢查是否為音訊類型（不區分大小寫）
                        audio_extensions_lower = [ext.lower() for ext in CONFIG['AUDIO_EXTENSIONS']]
                        if file_ext in audio_extensions_lower:
                            found_files.append({
                                'type': 'audio', 
                                'path': full_path,
                                'level': level,
...
def process_html_for_media(system_path):
    ...
    for i, m3u8_url in enumerate(m3u8_urls, 1):
        ...
        if found_files:
            ...
            # 設定新的活動類型（根據配置決定）
            if best_file['type'] == 'video':
                result['new_activity_type'] = '影音教材_影片'
            elif best_file['type'] == 'audio':
                result['new_activity_type'] = '影音教材_音訊'
            
            success_msg = f"成功找到影音檔案 - 檔案名: {filename}{best_file['extension']}, 類型: {best_file['type']}, 路徑: {best_file['path']}"
            print(f"  🎉 {success_msg}")
            ...
            # 記錄未找到檔案的詳細信息
            all_extensions = CONFIG['VIDEO_EXTENSIONS'] + CONFIG['AUDIO_EXTENSIONS']
            extensions_str = '/'.join(all_extensions)
            error_msg = f"未找到影音檔案 - 搜索檔案名: {filename}{extensions_str}, HTML: {system_path}, m3u8: {m3u8_url}"
            print(f"  ❌ 未找到檔案: {filename}{extensions_str}")
            logger.warning(error_msg)
    
    # 所有m3u8都處理完仍未找到檔案
    final_error = f"處理完所有m3u8路徑後未找到影音檔案: {system_path}"
    print(f"  ❌ 未找到對應的影音檔案")
...
def process_activity_for_media(activity_type, system_path):
    ...
    Returns:
        dict: {
            ...
            'new_file_path': str,
            'use_web_path': bool,
            'fallback_to_online': bool,  # 是否需要回退到線上連結
    ...
    logger.info(f"處理學習活動影音識別 - 活動類型: {activity_type}, 系統路徑: {system_path}")
    
    # 檢查是否需要處理
    if not should_process_activity(activity_type, system_path):
        logger.info(f"學習活動不需要影音檔案處理，返回預設結果")
    ...
    if media_result['found']:
        ...
        logger.info(f"影音檔案識別成功 - 新活動類型: {result['new_activity_type']}, 檔案路徑: {result['new_file_path']}")
        return result
    else:
        # 未找到影音檔案，需要回退到線上連結
        result = {
            ...
            'error_message': media_result.get('error_message', '未找到對應影音檔案')
        }
        logger.info(f"影音檔案識別失敗，回退到線上連結模式: {system_path}, 錯誤: {result['error_message']}")
        return result