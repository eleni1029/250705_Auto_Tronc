...
"""
Excel 課程結構生成器 - 主要生成器
掃描 04_manifest_structures 資料夾，將 JSON 課程結構轉換為 Excel 檔案
"""
...
class ExcelGenerator:
    ...
    def __init__(self, json_dir: str = "04_manifest_structures", output_dir: str = "05_to_be_executed"):
        """
        初始化生成器
...
def main():
    ...
    # 取得用戶輸入
    while True:
        print("請輸入 JSON 檔案資料夾名稱 (輸入 '0' 使用預設: 04_manifest_structures): ", end="", flush=True)
        json_folder = input().strip()
        if not json_folder:
            print("⚠️ 請輸入有效值，或輸入 '0' 使用預設值")
            continue
        if json_folder == '0':
            json_folder = "04_manifest_structures"
